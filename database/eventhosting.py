...
def authenticate_admin(engine,
    ...
    hash_obj.update(q_password.encode())
    q_password = hash_obj.hexdigest()"""
    query = database.select(ADMIN).where(
        (ADMIN.USERNAME == q_username) & (ADMIN.PASSWORD == q_password))
    with Session(engine) as session:
        try:
...
def authenticate_student(engine,
    ...
    hash_obj.update(q_password.encode())
    q_password = hash_obj.hexdigest()"""
    query = database.select(STUDENT).where(
        (STUDENT.USERNAME == q_username) & (STUDENT.PASSWORD == q_password))
    with Session(engine) as session:
        try:
            authenticated = True if len(session.scalars(query).all()) == 1 else False
        except Exception as e:
...
def get_students(engine, q_major: List[str] = ["%"], q_citizenship: List[str] = ["%"],
                 q_race: List[str] = ["%"], q_gender: List[str] = ["%"]) -> List[
    STUDENT]:
    query = database.select(STUDENT).filter(
        (or_(*[STUDENT.MAJOR.like(major) for major in q_major])) &
        (or_(*[STUDENT.CITIZENSHIP.like(citizenship) for citizenship in
               q_citizenship])) &
...
def insert_invite(engine,
                  q_eid: int, q_sid: List[int]) -> None:
    with Session(engine) as session:
        try:
            student_id_query = session.query(STUDENT).filter(
                STUDENT.SID.in_(q_sid)).all()
            for student in student_id_query:
                invited_to = INVITEDTO(I_SID=student.SID,
                                       I_EID=q_eid,
                                       ATTENDING=None)
    ...
    with Session(engine) as session:
        ...
        try:
            if q_username is not None:
                student_id_query = database.select(STUDENT).where(
                    STUDENT.USERNAME == q_username)
                student_id = session.scalars(student_id_query).one()
                student_id = asdict(student_id)
                student_id = student_id["SID"]
                invite_query = database.select(INVITEDTO, EVENT).where(
                    INVITEDTO.I_SID == student_id).join(EVENT,
                                                        EVENT.EID == INVITEDTO.I_EID)
